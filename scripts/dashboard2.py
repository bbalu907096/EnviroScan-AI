...
>>>>>>> e49ab38c4c8e361850e2c0dd64c76f356c6f68d4
...
if not high_risk.empty:
    ...
    st.dataframe(high_risk[["location_id","pollution_source","aqi"]])
else:
...
for _, row in df_filtered_pollutant.iterrows():
    folium.CircleMarker(
        ...
        fill=True,
        fill_opacity=0.7,
        popup=f"Location: {row['location_id']}<br>{selected_pollutant}: {row[selected_pollutant]:.2f}<br>Source: {row['pollution_source']}"
    ).add_to(marker_cluster)

# Add legend for pollutant intensity
...
# Filter rows with valid latitude, longitude, and AQI
...
# Add HeatMap
HeatMap(heat_data, min_opacity=0.4, max_opacity=0.8, radius=25, blur=15, gradient={0.2: 'green', 0.5: 'orange', 0.8: 'red'}).add_to(m_aqi)

# Add legend for AQI severity
legend_html_aqi = """
<div style="position: fixed; 
